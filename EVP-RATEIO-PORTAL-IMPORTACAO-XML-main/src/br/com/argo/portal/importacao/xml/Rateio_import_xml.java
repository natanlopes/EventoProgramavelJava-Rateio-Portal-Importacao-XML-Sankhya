package br.com.argo.portal.importacao.xml;
...
public class Rateio_import_xml implements AcaoRotinaJava {
...
//	            ctx.setMensagemRetorno("A operação não pode ser executada pois já existe rateio para o NUNOTA: " + nUnico + ".");
	            ...
	            if (!rateioOrigem.isEmpty()) {
	                ...
	                ctx.setMensagemRetorno("Parabens o  foi rateado com sucesso: " + nUnico + ".");
	                ...
	                ctx.setMensagemRetorno("Não há rateio para o NUNOTA nem na nota de origem: " + nUnico + ".");
	            }
	        }
	    } catch (Exception e) {
	        e.printStackTrace();
	        throw new Exception("Erro ao processar a ação: " + e.getMessage());
	    }
	}
	...
	private List<Map<String, Object>> buscarNunfi(BigDecimal nuNota) throws Exception {
		...
		try {
			...
			query.appendSql("SELECT\r\n"
					...
					+ "--	AND PED.QTDNEG <> 0\r\n"
					+ "GROUP BY\r\n"
					+ "	RAT.ORIGEM,	RAT.NUFIN , 	RAT.CODNAT, 	RAT.CODCENCUS, 	RAT.CODPROJ, 	RAT.NUMCONTRATO, 	RAT.CODPARC,RAT.CODCTACTB,\r\n"
					+ "	RAT.PERCRATEIO, 	RAT.DIGITADO, 	RAT.DTALTER , 	RAT.CODUSU, 	RAT.CODSITE");
